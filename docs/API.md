        ...
        "windows": [
          {
            "kind": "session",
            "windowId": "five-hour",
            "cycleId": "optional-provider-cycle-id",
            "usedPercent": 42,
            "remainingPercent": 58,
...
Authenticated SaaS stats may include `notificationRulesUpdatedAt`. This is only a version stamp: clients re-read `/api/notification-rules` when it changes. The rule document and push token are never embedded in a stats or SSE frame.
...
`limits.providers[].accountKey` is the legacy hashed account identifier (`sha256:…`) used to dedupe the same account across devices. `accountIdentity` is an optional, hashed, cross-device provider subject; when present, aggregation uses it ahead of `accountKey`. Collectors only emit it when the provider supplies a genuine account/workspace identity — never by guessing from an account label, a local path, or a rotating credential. `accountEmail` is the account email when available, and `accountName` is a sanitized display/profile name. Codex may additionally send `workspaceKind: "personal"` when the workspace has no provider-supplied name, allowing account-management UI to localize the Personal label without persisting translated text. `accountLabel` is the legacy provider-defined short label retained for mixed-version compatibility: older OpenCode renderers use it as the profile name, while existing providers may use it for the plan. `planLabel` is the explicit plan label (for example `Plus`, `Go`, or `Zen`) when identity and plan must be carried separately; readers fall back to `accountLabel` for payloads produced before `planLabel` existed. These fields MAY be sent to the authenticated hub so devices can identify each account and its plan. The hub ingest is protected by the shared `secret`; the **public** stats endpoints (`publicLimits`) strip `accountKey`, `accountIdentity`, `accountEmail`, `accountName`, `accountLabel`, `planLabel`, `workspaceKind`, the third-party `balance.quotaGroup`, and `balance.tranches` so neither account identity, plan labels, custom group labels, nor per-grant credit detail are exposed publicly.

`limits.providers[].windows[].windowId` is an optional, provider-defined stable quota-window key (for example Codex `primary` / `secondary`, or Cursor `total` / `auto`). It is a protocol identifier, not a localized label; clients must not derive notification-rule targets from `label`. `cycleId` is optional and is emitted only when an upstream provider exposes a real cycle identifier. Older payloads may omit all three stable fields; readers must then retain the established `accountKey` behavior and treat any window fallback as legacy rather than fabricating an identifier from its label.
